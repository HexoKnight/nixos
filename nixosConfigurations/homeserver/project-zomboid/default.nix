...
let
  ...
  dedicated-server = pkgs.callPackage ./dedicated-server.nix {};
...
in
{
  config = {
    ...
    environment.systemPackages = [ dedicated-server ];
    ...
    systemd = {
      ...
      services.zomboid = {
        ...
        serviceConfig = {
          ...
          ExecStart = "${lib.getExe dedicated-server} -cachedir=${dataDir}";
