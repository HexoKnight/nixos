# upstream except where specified
{
  config,
  lib,
  pkgs,
...
in
{
  ...
  options = {
    ...
    systemd = lib.mkOption {
      ...
      default = { };
      internal = true;
      description = "final systemd config";
    };
    networking = lib.mkOption {
      type = lib.types.attrsOf lib.types.anything;
      default = { };
      internal = true;
      description = "final networking config";
    };
  };

  config = lib.mkIf cfg.enable {
    # CHANGED
    systemd.services."factorio-${name}" = {
      ...
      # CHANGED
      unitConfig.RequiresMountsFor = [ "/var/lib/private/${cfg.stateDirName}" ];
    };
