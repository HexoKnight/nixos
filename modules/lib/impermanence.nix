...
let
  ...
  original-options = lib.makeExtensibleWithCustomName "override" (
    final:
    let
      coercedToSubmoduleOpts =
        attrName: options: listOf (coercedTo str (f: { ${attrName} = f; }) (final.submoduleOpts options));
    in
    {
  ...
  options = original-options.override (
    final: prev: {
      # TODO: add extensions
    }
  );
