    ...
    lib.mapAttrsToList (
      ...
      if builtins.isString value then
        let
          ...
          luaSetupName = if customLuaSetupName != null then customLuaSetupName else name;

          type = if isLua then "lua" else "viml";
          config =
            if !isLua then
              value
            else if !doLuaSetup then
              value
            else
              ''
        ...
        let
          plugin = lib.pipe (value.plugin or pkgPlugin) (
            lib.optional (value ? override) (p: p.overrideAttrs value.override)
        ...
        throw "plugin attr '${name}' must be a string or attrset"
    );
