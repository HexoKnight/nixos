...
let
  charFlags =
    ...
    let
      isCharAllowed =
        if lib.isFunction charTest then
          charTest
        else if lib.isString charTest then
          lib.flip lib.elem (lib.stringToCharacters charTest)
        else
          abort "`charTest` must be a function or a string";
      isValid =
        str:
        let
          ...
          { name, config, ... }:
          {
            options = {
              ...
              __full = lib.mkOption {
                ...
                type = types.str;
                default =
                  if lib.elem "m" (lib.stringToCharacters config.flags) then
                    assert (config.args == "" || throw "mouse bind ('m' flag) cannot have args");
                    config.dispatcher
                  else
                    "${config.dispatcher}, ${config.args}";
              };
            };
          }
        );
    ...
    wayland.windowManager.hyprland.settings =
      let
        ...
        toBindArg = keybind: bindAction: {
          ...
          arg =
            assert (
              (builtins.match "[^,]*,[^,]+" keybind) != null
              || throw "keybind (${keybind}) must contain exactly one comma seperator"
            );
            "${keybind}, ${bindAction.__full}";
        };
        binds = lib.concatLists (
          lib.mapAttrsToList (keybind: actions: map (toBindArg keybind) (lib.toList actions)) hyprbinds
        );
        groupedBinds = lib.groupBy (bind: bind.flags) binds;
        bindAttrset = lib.mapAttrs' (
          flags: binds: lib.nameValuePair "bind${flags}" (map (bind: bind.arg) binds)
        ) groupedBinds;
      in
      bindAttrset;
  };
