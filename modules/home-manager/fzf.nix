...
let
  ...
  genFzfCommand =
    ...
    let
      setDefaultCommand =
        if defaultCommand == null then "" else lib.toShellVar "FZF_DEFAULT_COMMAND" defaultCommand;
      ...
      commandLineArgs = genCommandLineArgs (
        options
        // {
    ...
    rec {
      ...
      toggle =
        ...
        edit {
          args = true;
          "--" = path;
        } /* jq */ ''setpath($ARGS.positional; getpath($ARGS.positional) // false | not)'';
      toggle-flag =
        flag:
        toggle [
          "flags"
  ...
  genFzfbinds = lib.mapAttrsToList (
    key: actions:
    let
      ...
      actionList =
        lib.concatMap
          (
            ...
            map (
              ...
              let
                ...
                ) (throw "no valid delimiters for fzf bind action arg: '${actionArg}'") delimiterList;

                startDelimiter = lib.head delimiterPair;
                endDelimiter = lib.last delimiterPair;
              in
              if actionArg == null then
                actionName
              else
                lib.concatStrings [
                  actionName
          ...
          (
            if builtins.isAttrs actions && !actions ? name then
              lib.mapAttrsToList withArg actions
            else
              lib.toList actions
          );
